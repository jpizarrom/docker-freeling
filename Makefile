...
Dockerfile-es: Dockerfile.m4 $(DOCKERCFG) python.docker pyfreeling.docker es-config.docker
	svn export --trust-server-cert --non-interactive --force -q https://github.com/TALP-UPC/FreeLing.git/tags/4.1/APIs/python3 APIs/python3
	svn export --trust-server-cert --non-interactive --force -q https://github.com/TALP-UPC/FreeLing.git/tags/4.1/APIs/common APIs/common
